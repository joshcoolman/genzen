...
- **Drag-to-reorder AI Images gallery** (`main`): Added `sort_order double precision` column to `user_images` (migration `20260226000000`). New images get `sort_order = Date.now()/1000`; variations use `source.sort_order - 0.001` (no more timestamp fudge). New `update-image-order.server.ts` persists reorder. `use-images.ts` exposes `reorderImages()` with optimistic updates + rollback. `ImageCard` uses `useSortable` with grip handle on hover. Gallery wrapped in `DndContext + SortableContext` with fractional midpoint sort_order calc on drop.
