...
- **Permissive in, informed after** — the generator panel no longer caps the reference strip at the smallest selected model, which used to _delete_ staged images when you ticked a one-image model. Stage as many as you like; each model takes what its endpoint holds, the submit drops the rest, and the card says "1 of 5 images". The picker became a table (name, price, capacity) sorted by price, with rows that cannot hold the current set dimmed but still selectable (#341)
