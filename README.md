...
- **The group card says what is happening in it** — a generation started inside a group used to take the newest swatch slot and render as a filled swatch of nothing, then stay that way until an unrelated write forced a re-read. The strip is pictures only now, work in flight gets its own kind of slot, the count says ", 2 working", and the gallery's poll refreshes the groups when a pending member settles — so a card is watchable from top level. The `...` menu also gained **Move to group**: one group's contents into another, and the emptied one goes (#350)
