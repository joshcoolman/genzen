...
- 2026-08-29 — The clip card is one thumbnail: the player and both end frames are a single picture, so the chrome sits on its corners the way the gallery card's does — `...` top-left, select tick bottom-left, both always on. Continue moved beside the prompt and the model became a fact in the caption, which is what freed the picture. Three clips across instead of one (#534, #535, #536)
- 2026-08-29 — Clips go in groups, the same feature Images has: grouping moved to `src/features/groups/` and a `kind` column keeps the two namespaces disjoint, so a clip cannot join an image group. Add to group in the drawer, `?group=` opens one, a clip made inside a group is filed as it lands, and the group card is a 16:9 cover with a square strip and a count (#517)
- 2026-08-29 — Several clips at once on Video, and one verb: Trash. A tick top-left of every player, Escape or Deselect all to leave, and the whole set binned in one round trip through the gallery's own bulk trash. The first pass of #517 — grouping itself is still open
- 2026-08-29 — The clip card is one picture of the clip: the player, then its first and last frame flush at 50/50, all sized to the clip's own shape so nothing is letterboxed. The last frame _is_ Continue — click it and its prompt comes with it. An unplayed card is a poster and one play button, so a page of clips fetches no video until asked, and only one plays at a time. Facts read shape then duration; cost moved out to the Activity log (#530)
- 2026-08-29 — Sequence's run is arrangeable: it wraps instead of scrolling off-screen past three clips, dragging opens a slot the shape of a clip where it will land, and a drop means the same thing whichever direction you came from (it used to land after the target going right and before it going left, with no way to reach the end of the run). The clip picker shows each clip's first _and_ last frame, since what a clip cuts into is decided by the frame it ends on (#529)
