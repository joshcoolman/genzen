...
- 2026-09-04 — Select mode's verbs take over the sidebar rail on desktop. A bar fixed to the bottom of the viewport is easy to miss unless you already know it is there, and it sits in peripheral vision while you are looking at the grid; the rail is where you already look to move around. Images, Video and Trash each list their verbs once and render two surfaces — the rail above 48rem, the same drawer below it, which is now the mobile implementation rather than the only one. `RailOverride` is a context the chrome provides and a route writes to on mount, so the takeover lasts exactly as long as the selection and select mode is still a selection rather than a switch (#587)
