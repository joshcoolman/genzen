...
- **Video has three models and a picker** — LTX-2.5 Fast, MiniMax H3 and Flux 3, one at a time. Single-select is the money decision, not a simplification: a clip is 20-100x the price of a still, so the image panel's "tick four and fire" would be a $20 click. Adding the two forced a shape change, because they disagree with LTX about more than their names — Flux 3 puts first+last frame on a separate endpoint that calls the first one `start_image_url`, and H3's image endpoint has no aspect ratio at all, since the output follows the frame it is given. A mode is a descriptor of what its endpoint takes now, and the submit builds its request from that rather than from LTX's params hardcoded (#385, #387)
- **You can delete a clip** — a failed generation used to sit on the Video page forever and a generating one could not be stopped. It reuses the gallery's own delete, so a generating clip is cancelled at FAL, a failed one goes outright, and a finished one goes to Trash. Trash had to be told clips exist: its list ignored them while Empty Trash destroyed them anyway, so a binned clip would have been invisible in the one place that could restore it (#384)
- **Grok Imagine 2.0 is in the image lineup** — text-to-image and edit, up to three images, verified against FAL with the exact payload we send (#383)
- **You can see what went into a generation** — Activity's References block shows every image a run was given, and a quiet Details link under each card's prompt opens that record in a new tab. The lineage had been recorded all along and read through a keyhole: the same fact lands under `reference_image_ids` or `source_image_id` depending on the submit path, and only the first was ever displayed. Two of ten generations here showed no references at all despite having had one, and the other eight showed one image fewer than actually went to FAL. Nothing new is recorded, so every image already made lights up (#380, #381)
- **Click a thumbnail on Images and you get a plain lightbox** — scrim over the app, the picture centred, chevrons either side, an X, a counter, click outside to leave. It steps through what the grid is showing, so inside a group that is the group. It replaces the in-place preview that had held the click for three days: leaving the working surface visible sounded right and read wrong, because the UI you can still see cannot act on the image you are looking at. Explore's three-column overlay is untouched and now lives in Explore, renamed off "lightbox" — one name for two unlike things is what put the wrong one on Images in the first place (#378)
