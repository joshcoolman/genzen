...
- **Select mode reads as a mode** — the generator dock fades and goes `inert` while a selection is up, and the selection drawer moves onto the accent: green wash, one glow pulse on arrival, black buttons with no borders. No "n selected" label, Deselect all is a peer button, and Delete is "Trash n" in the ordinary style — it is a soft delete, and red is reserved for Trash's Delete Forever (#326)
- **Select mode starts from the card** — every thumbnail carries its tick in the bottom-left now, and clicking one picks that image and puts the grid in select mode. The toolbar toggle is gone: it asked you to turn a mode on before you could touch the picture you were looking at, and it was also the only thing saying selection existed. `selectMode` stopped being state — it is "something is selected", so Deselect all and Escape are the way out (#325)
- **Groups sort among the thumbnails** instead of floating above them. A group sits where its newest member would, so an active one stays with today's pictures and a finished one sinks past them. The grid used to render group cards as their own block, which pinned a group finished months ago above everything made since (#324)
...
- **Video** — a new route, and the prompt is its only required input. Text only and the model invents the shot; add a first frame and it animates a still you made; add a last frame and it solves the move between the two. LTX-2.5 on FAL, 6–20s, ~$0.09/sec, audio included. Aspect options follow the mode, because `auto` only exists where there is an image to match. Animate sits in the card's `...` menu. Clips land in the bucket like every other asset, and `/img/[id]` answers range requests now so they can be scrubbed (#305)
