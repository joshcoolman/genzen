...
- **Click a group's swatch strip to see the whole group** — it keeps its five columns and its first row; the card just grows taller, five across, for as many rows as the group needs. Strictly additive, so the five you were looking at never move. A group of five or fewer is already fully visible and gets no toggle; the ids are fetched on open, because the summary's six ride on every group write (#352)
- **The group card says what is happening in it, without moving** — a group with a generation or an upload in flight says ", 3 working" in its caption; the swatch strip stays five pictures and changes once, after the work lands. Counting the work is client-side, so it costs no round trip, and the summaries are re-read once per burst rather than once per image. **Upload to group** no longer throws thumbnails onto the top-level grid and takes them away — the row is born in the group. The `...` menu also gained **Move to group**: one group's contents into another, and the emptied one goes (#350)
- **Images lost its filter and its find; Upload leads the toolbar** — the origin pills and the Cmd-F overlay both went, on the same finding: a group is a scope you made on purpose and it is the only one worth having. Upload moved to the far left with a menu — plain, or straight into a group you pick — and inside a group it skips the menu and files into the open one (#348, find parked in #347)
- **One field fill, one row hover** — the tinted background on the aspect and model fields was hand-written in five modules (one in `srgb` while the rest were `oklab`) and missing from the controls beside them. It is `--field`/`--field-hover` now, across every form control. Menu and option rows got `--row-hover`, a wash of light: they had all been hovering to `--surface-raised` while sitting on `--surface`, a five-point step that registered as nothing (#346)
- **The generator panel got its rhythm** — Instructions became a gear on the header, the dock's X went (the sidebar toggles it, and an X on a panel that survives closing reads as "discard"), Generate prompt moved down beside Add prompt to split that row evenly, and the "Reference images" heading went. Every vertical gap in the column now reads one `--panel-rhythm` knob instead of the 12/8/4 three components had each picked alone (#345)
- **Menus took the model list's density** — the thumb and group menus were drawing 14px rows with 16px icons next to a model picker drawing the same kind of row at 12px; both are picker lines and now draw alike, canvas's context menu included. The audit behind it found the token scale is healthy (21 raw declarations, 13 of them in canvas) and what is missing is role naming — which token a picker row vs. body vs. metadata gets (#343, follow-up #344)
