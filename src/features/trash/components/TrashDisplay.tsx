...
export function TrashDisplay() {
  ...
  const linkedCount = linkedImageIds.size
  const deletableCount = images.length - linkedCount
              ...
              <AlertDialog>
                <AlertDialogTrigger asChild>
                  <Button
                    variant="destructive"
                    size="sm"
                    disabled={isEmptying || deletableCount === 0}
                  >
                ...
                <AlertDialogContent>
                  <AlertDialogHeader>
                    ...
                    <AlertDialogDescription>
                      {linkedCount > 0 ? (
                        <>
                          This will permanently delete {deletableCount}{' '}
                          {deletableCount === 1 ? 'item' : 'items'} and their
                          files. {linkedCount} linked{' '}
                          {linkedCount === 1 ? 'item' : 'items'} will be kept
                          because active images depend on{' '}
                          {linkedCount === 1 ? 'it' : 'them'}. This action
                          cannot be undone.
                        </>
                      ) : (
                        <>
                          This will permanently delete {images.length}{' '}
                          {images.length === 1 ? 'item' : 'items'} and their
                              ...
                              {isLinked && (
                                <Tooltip>
                                  <TooltipTrigger asChild>
                                    <Badge
                                      variant="outline"
                                      className="shrink-0 gap-1 border-amber-500/50 text-amber-500"
                                    >
                                  ...
                                  <TooltipContent>
                                    Used by {depCount} active{' '}
                                    {depCount === 1 ? 'image' : 'images'} --
                                    restore or delete dependents first
                                  </TooltipContent>
                                </Tooltip>
                              )}
                            </div>
            ...
            <AlertDialogContent>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  ...
                  {selection.count === 1 ? 'item' : 'items'} and their files.
                  Linked items will be skipped. This action cannot be undone.
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
                <AlertDialogCancel>Cancel</AlertDialogCancel>
