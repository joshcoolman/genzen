...
- **A clip's thumbnail is a `<video>`, via `MediaBox`** (`#/components`), because an mp4 in an `<img>` lands on the broken-file fallback. Clips have had a real `thumbnail_path` since #499 and no surface reads it yet — #500 is the switchover, and it has to keep this path for clips made before that, which were not backfilled. The detail panel's large preview gets `controls`; the two square boxes do not. That component exists because the decision had been made three times (Video's card, Trash's row, then here).
