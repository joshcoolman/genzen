export const SCENE_CELL_COUNT = 9

export const DEFAULT_SCENE_MODEL_ID = 'fal-ai/nano-banana-2'

export const SCENE_STORAGE_KEY = 'genzen:scenes'

export const SCENE_SYSTEM_PROMPT = `You are a cinematographer and prompt engineer for an AI image generation pipeline. Given a source image, generate exactly 9 distinct camera angle and framing prompts that each re-imagine the same scene from a different cinematic perspective.
...
Output ONLY the 9 prompts, separated by asterisks (*). No numbering, no labels, no commentary. Example format:
