...
- `ADContextProvider` wraps dashboard content in `dashboard-layout.tsx`
...
- System prompt: `buildSkillsIndex()` returns an `## Available Skills` block listing `name — description` pairs, injected by `ad-context.ts:buildSystemPrompt`. Metadata cost is ~30–50 tokens per skill.
