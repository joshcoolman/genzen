...
- `server/fetch-models.server.ts` -- TanStack server fn hitting `api.fal.ai/v1/models` with cursor pagination (48 per page)
- `server/fetch-pricing.server.ts` -- TanStack server fn for per-model pricing lookup
- `hooks/use-models.ts` -- client hook with module-level cache, 300ms debounced search, and paginated loading
- `components/ModelsPage.tsx` -- main page layout with search bar, category tabs, model grid, and load-more
- `components/ModelCard.tsx` -- individual model display with endpoint copy, external link, and 12 muted color schemes
...
- ModelCard has 12 hardcoded muted color schemes via `colorIndex` prop; RecentlyAddedCarousel currently only uses colorIndex=0
