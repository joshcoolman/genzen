...
const SCENES = [
  'a quiet cafe scene',
  'a busy street corner in Tokyo',
  'a dog waiting by a door',
  'a mechanic under a car hood',
  'someone reading on a park bench',
  'a barber at work',
  'a fisherman at dawn',
  'a chef plating a dish',
  'a couple arguing in a diner',
  'a child discovering snow for the first time',
  'a boxer between rounds',
  'an executive nervous before a board meeting',
  'a street musician playing saxophone',
  'a woman at a laundromat at night',
  'a cyberpunk bar scene',
  'an old bookshop interior',
  'a surfer watching the ocean',
  "a farmer's market in morning light",
  'a taxi driver at 3am',
  'an astronaut looking at Earth',
  'a rainy window reflection',
  'a cat in a sunbeam',
  'a construction worker on a high beam',
  'a dancer mid-leap',
  'a lighthouse in a storm',
  'someone cooking in a tiny apartment',
...
export const generatePrompt = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data }) => {
    ...
    const photographerRef =
      Math.random() < 0.3 ? `, in the style of ${pick(PHOTOGRAPHERS)}` : ''
