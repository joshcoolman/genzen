...
// We need to mock h3 and the server modules before importing the handler.
// Because the handler uses module-level state, we re-import it fresh each test
...
vi.mock('h3', () => ({
  ...
  getHeader: (event: { _headers: Record<string, string> }, name: string) =>
    event._headers[name] ?? '',
  readRawBody: (event: { _rawBody: string }) => Promise.resolve(event._rawBody),
...
// Helper: import the handler fresh (resets module-level state)
async function importHandler() {
  const mod = await import('./fal-webhook.post')
...
describe('fal-webhook JWKS failure counter recovery', () => {
  ...
  it('processes requests normally after transient JWKS failures below the threshold', async () => {
    ...
    const result1 = await handler(makeSignedEvent(privateKey, rawBody))
    ...
    const result2 = await handler(makeSignedEvent(privateKey, rawBody))
  ...
  it('invalidates cache when failure threshold is exceeded so the next request forces a fresh JWKS fetch', async () => {
    ...
    await handler(makeSignedEvent(privateKey, rawBody))
    await handler(makeSignedEvent(privateKey, rawBody))
    expect(fetchCallCount).toBe(2)

    // Call 3: hits threshold → 401 AND cache must be invalidated
    const result3 = await handler(makeSignedEvent(privateKey, rawBody))
    ...
    const result4 = await handler(makeSignedEvent(privateKey, rawBody))
  ...
  it('resets the failure counter on successful JWKS fetch so threshold must be reached fresh each cycle', async () => {
    ...
    const r1 = await handler(makeSignedEvent(privateKey, rawBody)) // fetchCallCount=1 fails
    const r2 = await handler(makeSignedEvent(privateKey, rawBody)) // fetchCallCount=2 fails
    ...
    const r3 = await handler(makeSignedEvent(privateKey, rawBody)) // fetchCallCount=3 succeeds
    expect(r3).toBe('OK')
    expect(fetchCallCount).toBe(3)
    ...
    const ra = await handler2(makeSignedEvent(privateKey, rawBody))
    const rb = await handler2(makeSignedEvent(privateKey, rawBody))
    expect(ra).toBe('OK') // counter=1, below threshold
    expect(rb).toBe('OK') // counter=2, below threshold
    const rc = await handler2(makeSignedEvent(privateKey, rawBody))
