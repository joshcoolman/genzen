...
export function extractColors(pixels: Vec3[], count = 8): ExtractedColor[] {
  ...
  const diverse = filterByHueDiversity(quantized, minHueDistance, count + 4, true);
 ...
 */
export function extractColorsRandom(pixels: Vec3[], imageWidth: number, count = 8): ExtractedColor[] {
  const defaultColors: ExtractedColor[] = [
  ...
  const regionSize = Math.max(5, Math.min(20, Math.floor(Math.min(imageWidth, imageHeight) / 10)));
  ...
  for (let i = 0; i < count * 3 && result.length < count; i++) {
    ...
    const avgR = Math.round(regionPixels.reduce((s, p) => s + p[0], 0) / regionPixels.length);
    const avgG = Math.round(regionPixels.reduce((s, p) => s + p[1], 0) / regionPixels.length);
    const avgB = Math.round(regionPixels.reduce((s, p) => s + p[2], 0) / regionPixels.length);
    ...
    const hue = h * 360;
    const isFarEnough = result.every((c) => getHueDistance(c.hue, hue) >= 15 || s < 0.1);
...
function clusterColors(pixels: Vec3[], k: number, maxIter = 15): Vec3[] {
  ...
  for (let iter = 0; iter < maxIter; iter++) {
    ...
    for (let i = 0; i < centroids.length; i++) {
      ...
      const newR = Math.round(clusters[i].reduce((s, p) => s + p[0], 0) / clusters[i].length);
      const newG = Math.round(clusters[i].reduce((s, p) => s + p[1], 0) / clusters[i].length);
      const newB = Math.round(clusters[i].reduce((s, p) => s + p[2], 0) / clusters[i].length);
...
export function extractColorsQuadrant(
  ...
  const snappedColors = clusterCenters.map((center) => snapToNearestPixel(center, pixels));
  ...
  extracted.sort((a, b) => (b.vibrancy + Math.random() * 0.1) - (a.vibrancy + Math.random() * 0.1));
  ...
  for (const color of extracted) {
    ...
    const isDiverse = diverse.every((existing) => {
      ...
      return minDiff >= 20 || color.saturation < 0.1 || existing.saturation < 0.1;
    });
