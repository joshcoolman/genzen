...
import { samplePixels, extractColors, extractColorsRandom, extractColorsQuadrant } from "./quantize.ts";
...
Deno.serve(async (req) => {
  ...
  try {
    // Parse request body
    const { image_id, storage_path, user_id, mode = "quadrant" }: PaletteRequest = await req.json();

    if (!image_id || !storage_path || !user_id) {
      return new Response(
        JSON.stringify({ error: "Missing required fields: image_id, storage_path, user_id" }),
